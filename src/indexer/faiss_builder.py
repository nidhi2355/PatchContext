...
# Import processor if available in src/indexer/data_processor.py
try:
    from src.indexer.data_processor import RepositoryDataProcessor
except ImportError:
    RepositoryDataProcessor = None
...
class VectorStoreBuilder:
    ...
    def build_and_save_index(self, documents=None):
        """Processes repository chunks and builds persistent FAISS index."""
        chunks = documents
        
        # Fall back to RepositoryDataProcessor if documents are not passed directly
        if chunks is None:
            if RepositoryDataProcessor is not None:
                processor = RepositoryDataProcessor()
                chunks = processor.process_and_chunk_repository()
            else:
                print("Error: No documents provided and RepositoryDataProcessor is unavailable.")
                return
